...
export function bookEndList(numbers: number[]): number[] {
    ...
    const isFirstOrLast = (num: number): boolean =>
        numbers.indexOf(num) === 0 ||
        numbers.indexOf(num) === numbers.length - 1;
    const firstAndLast = [...numbers].filter(isFirstOrLast);
    return firstAndLast;
}
...
export function stringsToIntegers(numbers: string[]): number[] {
    return [...numbers].map((num: string): number => Number(num) || 0);
}
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const isNotQuestioning = (message: string): boolean =>
        !message.endsWith("?");
    return [...messages]
        .map((message: string): string =>
            message.endsWith("!") ? message.toUpperCase() : message,
        )
        .filter(isNotQuestioning);
};
...
export function countShortWords(words: string[]): number {
    const isShort = [...words].filter(
        (word: string): boolean => word.length < 4,
    );
    return isShort.length;
}
/**
